...
/******************************************************************************
 * Reproducible Annual Environmental Variable Table Generator
 * Google Earth Engine JavaScript API / Code Editor
 *
 * Description
 * -----------
 * This script generates a single-year, grid-aligned CSV table from multiple
 * Google Earth Engine datasets. All variables are resampled to a user-defined
 * WGS84 target grid and exported as one row per valid grid cell.
 *
 * Variables
 * ---------
 *   - NDVI: growing-season or annual mean NDVI from MODIS MOD13A1
 *   - NPP: annual net primary productivity from MODIS MOD17A3HGF
 *   - Precipitation: annual precipitation from ERA5-Land Daily Aggregated
 *   - ShortwaveRadiation: annual downward shortwave radiation from ERA5-Land
 *   - Temperature: annual mean 2 m air temperature from ERA5-Land
 *   - Elevation: SRTM elevation
 *   - LandCover: MODIS MCD12Q1 IGBP land-cover class
 *   - SOC: soil organic carbon from OpenLandMap, averaged over 0, 10, and 30 cm
 *
 * Reproducibility notes
 * ---------------------
 *   1. The target grid is explicitly defined by CRS and affine transform.
 *   2. Dataset IDs, scale factors, temporal windows, masks, and export settings
 *      are centralized in CONFIG and DATASETS.
 *   3. Each exported row contains run metadata, including script version,
 *      analysis ID, target year, grid resolution, CRS, and NDVI month window.
 *   4. No random sampling is used.
 *   5. For full reproducibility, record the exact study-area asset version or
 *      keep a copy of the boundary geometry in the repository.
 *
 * Required user action
 * --------------------
 * Replace CONFIG.studyAreaAsset with your own Earth Engine FeatureCollection
 * asset path before running the script.
 *
 * Author: <your name>
 * Repository: <your GitHub repository URL>
 * License: MIT, Apache-2.0, or your preferred license
